...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
	...
	// This if block checks to see if the parent and child are null and checks to make
	// sure that the parent and child are related.
	if (child == null || parent == null || child.up != parent){
		throw new IllegalArgumentException("These nodes are not related");
	}
	
	Node<T> gramps = parent.up; // Creates a reference to the parent of the root
				    // of the rotation, to maintain order 

	if (parent.down[0] == child){  // selects the left child if applicable
		parent.down[0] = child.down[1]; // flips the edge of the child node being rotated
		if (child.down[1] != null){ 
			child.down[1].up = parent; // adds an edge if the rotation requires a child
						   // with a new parent
		}
		child.down[1] = parent; // The right child of the child moves up
		parent.up = child; // Child becomes the parent
	}
	else if (parent.down[1] == child){ // selects the right child if applicable
		parent.down[1] = child.down[0]; // flips the edge of the child node being rotated
		if (child.down[0] != null){
			child.down[0].up = parent; // adds an edge if the rotation requires a child
                                                   // with a new parent
		}
		child.down[0] = parent; // The left child of the child moves up
		parent.up = child; // Child becomes the parent
	}
	else{
		//checks for relation for either right or left rotaion
		throw new IllegalArgumentException("These nodes share no relation");
	}
	
	// If the grandparent node exists this block makes it point to child to ensure continuity
	if (gramps != null){
		if (gramps.down[0] == parent){
			gramps.down[0] = child;
		}else{
			gramps.down[1] = child;
		}
	...
	child.up = gramps; // This ensures the child points to its new parent
    ...
    // Test 1 tests a right rotation on the root node
    public static boolean test1() {
        // TODO: Implement this test.
	BinarySearchTree<Integer> bst = new BinarySearchTree<>(); //Creates the test tree object

	// Inserts the nodes to build original tree
	bst.insert(25);
	bst.insert(9);
	bst.insert(85);
	bst.insert(1); 
	
	bst.rotate(bst.root.down[0], bst.root); // Performs a right rotation on the root node making the root 9 
	
        return bst.toLevelOrderString().equals("[ 9, 1, 25, 85 ]"); // Checks the BST verus the correct solution
    }
    
    // Test 2 tests a double rotation moving the node furthest from the root to the root
    public static boolean test2() {
        // TODO: test a standard binary search tress
        BinarySearchTree<Integer> bst = new BinarySearchTree<>(); //Creates the test tree object
	...
	// Inserts the nodes to build original tree
        ...
        bst.rotate(bst.root.down[0].down[1], bst.root.down[0]); // Performs a left roatation on the lowest Node (21) 
	bst.rotate(bst.root.down[0], bst.root); // Performs a right rotation on Node (21) making it the root

        return bst.toLevelOrderString().equals("[ 21, 18, 37, 52 ]"); // Checks the BST verus the correct solution
    }
    
    // Test 3 tests a rotation at the root and one on each side of the BST
	...
	bst.rotate(bst.root.down[1], bst.root); // Performs a left rotation on the root 87, making the new root 88
	bst.rotate(bst.root.down[1].down[0], bst.root.down[1]); // Performs right rotation on Node (90)
	bst.rotate(bst.root.down[0].down[0].down[0], bst.root.down[0].down[0]); // Performs a right rotation on Node (10)

        return bst.toLevelOrderString().equals("[ 88, 87, 90, 10, 95, 9, 69, 97 ]"); // checks the BST verus the correct solution
    }
